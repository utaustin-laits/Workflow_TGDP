...
import de.linguisticbits.workflow.data.ELANMessage.ELANMessageType;
...
public class ELANChecker {
    ...
    private List<ELANMessage> checkFilename(File eafFile) {
        ...
        if (result.isEmpty()){
            result.add(new ELANMessage(ELANMessageType.OK, "No errors in filename", eafFile));                                    
        }
        return result;        
    }
    ...
    private List<ELANMessage> checkAudio(File eafFile, Document eafDoc) throws JDOMException {
        List<ELANMessage> result = new ArrayList<>();
        /*
        ...
        */
        ...
        if (mediaDescriptor==null){
            ...
            String mediaURL = mediaDescriptor.getAttributeValue("MEDIA_URL").replaceAll(" ", "%20");
            try {
                ...
                result.add(new ELANMessage(ELANMessageType.WARNING, "URISyntaxException: " + ex.getMessage(), eafFile));                                                
            } catch (MalformedURLException ex) {
                result.add(new ELANMessage(ELANMessageType.WARNING, "MalformedURLException: " + ex.getMessage(), eafFile));                                                
            }
        }
        if (result.isEmpty()){
            result.add(new ELANMessage(ELANMessageType.OK, "No errors in audio", eafFile));                                    
        }        
        return result;        
    }
    ...
    private List<ELANMessage> validate(File eafFile) {
        ...
        if (result.isEmpty()){
            result.add(new ELANMessage(ELANMessageType.OK, "No errors in schema validation", eafFile));                                    
        }        
    ...
    private List<ELANMessage> checkLinguisticTypes(File eafFile, Document eafDoc) throws JDOMException {
        ...
        if (lingTypeTranscription==null){
            ...
            if (!("true").equals(timeAlignable)){
                String message = "LINGUISTIC_TYPE 'TRANSCRIPTION' defined as time-alignable='true'.";
                result.add(new ELANMessage(ELANMessageType.ERROR, message, eafFile));                            
            }
        }
        ...
        if (result.isEmpty()){
            result.add(new ELANMessage(ELANMessageType.OK, "No errors in linguistic types (tier structure)", eafFile));                                    
        }        
    ...
    private List<ELANMessage> checkSchema(File eafFile, Document eafDoc) {
        ...
        if (result.isEmpty()){
            result.add(new ELANMessage(ELANMessageType.OK, "No errors in schema reference", eafFile));                                    
        }        
    ...
    private List<ELANMessage> checkTiers(File eafFile, Document eafDoc) throws JDOMException {
        ...
        */
        ...
        if (index1<0 || index2<0){
            result.add(new ELANMessage(ELANMessageType.WARNING, "Cannot check tiers against filename because filename does not match pattern.", eafFile));                                                
            return result;
        }
        ...
        if (result.isEmpty()){
            result.add(new ELANMessage(ELANMessageType.OK, "No errors in tiers", eafFile));                                    
        }        
    ...
    private List<ELANMessage> checkSpeakers(File eafFile, Document eafDoc) throws JDOMException {
        ...
        */
        ...
        if (result.isEmpty()){
            result.add(new ELANMessage(ELANMessageType.OK, "No errors in speakers", eafFile));                                    
        }        
