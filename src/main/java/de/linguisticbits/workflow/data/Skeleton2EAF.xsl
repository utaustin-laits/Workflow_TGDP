...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="SPEAKER_NUMBERS">1,2,N345</xsl:param>
    <xsl:param name="INTERVIEWER_NUMBERS">3,4</xsl:param>
    <xsl:param name="AUDIO_PATH"
        >file:///D:/tgdp_20230210/prod/sound_files/1-1-1/1-1-1-3-a.wav</xsl:param>
    ...
    <xsl:template match="TIME_ORDER">
        ...
        <xsl:for-each select="tokenize($SPEAKER_NUMBERS, ',')">
            <xsl:variable name="CLEAN_SPEAKER_NUMBER">
                <xsl:choose>
                    <xsl:when test="starts-with(normalize-space(.), 'N')">
                        <xsl:value-of
                            select="concat('N', format-number(xs:integer(substring-after(normalize-space(.), 'N')), '000'))"
                        />
                    </xsl:when>
                    <xsl:otherwise>
            ...
            <TIER DEFAULT_LOCALE="en" LINGUISTIC_TYPE_REF="TRANSCRIPTION">
                <xsl:attribute name="PARTICIPANT" select="concat('Speaker_', $CLEAN_SPEAKER_NUMBER)"/>
                <xsl:attribute name="TIER_ID"
                    select="concat('TRANSCRIPTION_Speaker_', $CLEAN_SPEAKER_NUMBER)"/>
            </TIER>
            ...
            <TIER DEFAULT_LOCALE="en" LINGUISTIC_TYPE_REF="TRANSLATION">
                ...
                <xsl:attribute name="PARTICIPANT" select="concat('Speaker_', $CLEAN_SPEAKER_NUMBER)"/>
                <xsl:attribute name="TIER_ID"
                    select="concat('TRANSLATION_Speaker_', $CLEAN_SPEAKER_NUMBER)"/>
            </TIER>
        </xsl:for-each>

        <xsl:for-each select="tokenize($INTERVIEWER_NUMBERS, ',')">
            <xsl:variable name="CLEAN_SPEAKER_NUMBER" select="xs:integer(normalize-space(.))"
                as="xs:integer"/>
            <!-- <TIER DEFAULT_LOCALE="en" LINGUISTIC_TYPE_REF="TRANSCRIPTION" PARTICIPANT="Speaker_0001"
                    TIER_ID="TRANSCRIPTION_Speaker_0001"> -->
            <TIER DEFAULT_LOCALE="en" LINGUISTIC_TYPE_REF="TRANSCRIPTION">
                <xsl:attribute name="PARTICIPANT"
                    select="concat('Interviewer_', format-number($CLEAN_SPEAKER_NUMBER, '0000'))"/>
                <xsl:attribute name="TIER_ID"
                    select="concat('TRANSCRIPTION_Interviewer_', format-number($CLEAN_SPEAKER_NUMBER, '0000'))"
                />
            </TIER>
            ...
            <TIER DEFAULT_LOCALE="en" LINGUISTIC_TYPE_REF="TRANSLATION">
                ...
                <xsl:attribute name="PARTICIPANT"
                    select="concat('Interviewer_', format-number($CLEAN_SPEAKER_NUMBER, '0000'))"/>
                <xsl:attribute name="TIER_ID"
                    select="concat('TRANSLATION_Interviewer_', format-number($CLEAN_SPEAKER_NUMBER, '0000'))"
                />
            </TIER>
        </xsl:for-each>
